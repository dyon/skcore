...
class icecrown_citadel_teleport : public GameObjectScript
{
    public:
        ...
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Teletransporte a Martillo de la Luz.", GOSSIP_SENDER_ICC_PORT, LIGHT_S_HAMMER_TELEPORT);
            if (InstanceScript* instance = go->GetInstanceScript())
            {
                if (instance->GetBossState(DATA_LORD_MARROWGAR) == DONE || player->isGameMaster())
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Teletransporte a Oratorio de los Malditos.", GOSSIP_SENDER_ICC_PORT, ORATORY_OF_THE_DAMNED_TELEPORT);
