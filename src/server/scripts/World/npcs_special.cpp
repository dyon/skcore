...
public:
        ...
        if (player->HasAchieved(ACHI_PONY_UP))
            if (!creature->HasAura(SPELL_SQUIRE_TIRED))
            {
                ...
                if (uiBuff & STATE_BANK)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "Ver el banco.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_BANK);
                if (uiBuff & STATE_SHOP)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, "Visitar al mercader.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);
                if (uiBuff & STATE_MAIL)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Echar un vistazo al buz""\xC3\xB3""n.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_MAIL);
            }

        // Horde
        if (player->GetQuestRewardStatus(QUEST_CHAMP_SENJIN))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de los Lanza Negra", GOSSIP_SENDER_MAIN, SPELL_SENJIN_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_UNDERCITY))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Entra""\xC3\xB1""", GOSSIP_SENDER_MAIN, SPELL_UNDERCITY_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_ORGRIMMAR))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Orgrimmar", GOSSIP_SENDER_MAIN, SPELL_ORGRIMMAR_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_SILVERMOON))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Lunargenta", GOSSIP_SENDER_MAIN, SPELL_SILVERMOON_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_THUNDERBLUFF))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Cima del Trueno", GOSSIP_SENDER_MAIN, SPELL_THUNDERBLUFF_PENNANT);

        //Alliance
        if (player->GetQuestRewardStatus(QUEST_CHAMP_DARNASSUS))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Darnassus", GOSSIP_SENDER_MAIN, SPELL_DARNASSUS_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_EXODAR))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Exodar", GOSSIP_SENDER_MAIN, SPELL_EXODAR_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_GNOMEREGAN))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Gnomeregan", GOSSIP_SENDER_MAIN, SPELL_GNOMEREGAN_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_IRONFORGE))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Forjaz", GOSSIP_SENDER_MAIN, SPELL_IRONFORGE_PENNANT);
        if (player->GetQuestRewardStatus(QUEST_CHAMP_STORMWIND))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Estandarte de Campe""\xC3\xB3""n de Ventormenta", GOSSIP_SENDER_MAIN, SPELL_STORMWIND_PENNANT);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch (uiAction)
        {
            ...
            case GOSSIP_ACTION_MAIL:
                creature->SetFlag(UNIT_NPC_FLAGS, 0x04000000);              // Flag Mailbox
    ...
    struct npc_argent_squireAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (me->HasAura(SPELL_SQUIRE_TIRED) && me->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_BANKER | 0x04000000 | UNIT_NPC_FLAG_VENDOR))
            {
                me->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_BANKER | 0x04000000 | UNIT_NPC_FLAG_VENDOR);
